...
var idDeskInput = document.getElementById('id-desk');
...
function populateOptions(selectElement, dataObject) {
    ...
    for (var key in dataObject) {
        ...
        // Ensure dataObject[key] exists before accessing nom_desk:
        if (dataObject[key] && dataObject[key].hasOwnProperty("nom_desk")) {
            option.text = dataObject[key]["nom_desk"];
            option.value = dataObject[key].id;
        } else {
            option.text = key; // Handle cases where nom_desk might be missing
        }
        option.value = key;
...
// Event listener for desk select (moved here after the desk options are populated)

deskSelect.addEventListener('change', function () {
  var selectedOption = this.options[this.selectedIndex];
  var selectedDeskName = selectedOption.textContent.trim(); // Get the selected desk name
  var selectedDeskId = selectedOption.value;
  document.getElementById('selected-desk-name').value = selectedDeskName;
  // Retrieve the corresponding desk object and display its ID in the input field:
  var selectedWilaya = wilayaSelect.value;
  var selectedCommune = communeSelect.value;
  if (data[selectedWilaya] && data[selectedWilaya][selectedCommune]) {
      var desks = data[selectedWilaya][selectedCommune];

      // Find the desk object with the selected name
      var selectedDesk = desks.find(function(desk) {
          return desk.nom_desk === selectedDeskName;
      });

      if (selectedDesk) {
          idDeskInput.value = selectedDesk.id; // Populate ID desk input with the ID of the selected desk
          selectedDeskId=selectedDesk.id;
          console.log("Selected Desk name: ", selectedDeskName);
          console.log("Selected Desk ID: ", selectedDeskId);
      } else {
          console.error("Desk not found with name:", selectedDeskName);
      }
  } else {
      console.error("Desk data not found for wilaya:", selectedWilaya, "and commune:", selectedCommune);
  }
});
 // Get references to elements
 var toggleSwitch = document.getElementById('point-relais-switch');
 var displayedInputs = document.querySelectorAll('.switch-oui');
 var switchValueInput = document.getElementById('switch-value');

 if (toggleSwitch && displayedInputs.length > 0) {
     // Function to toggle element display
     function toggleElements(display) {
         displayedInputs.forEach(function (input) {
             input.style.display = display ? 'block' : 'none';
         });
     }

     // Initially hide elements if the first radio button is checked (assuming it's "Non")
     toggleElements(document.querySelector('input[name="form[point_relais]"]:checked').value === "1");

     // Add event listener to the radio buttons
     toggleSwitch.addEventListener('change', function () {
         // Show elements if the checked radio button's value is "1" (Oui), hide otherwise
         toggleElements(this.querySelector('input:checked').value === "1");
         var switchValue = this.querySelector('input:checked').value;
         // Assign the switch value to the hidden input field
        switchValueInput.value = switchValue;
        console.log("switched value ",switchValueInput.value);
     });
 } else {
